...
def qconv3x3(in_planes, out_planes, stride=1, wbit=32, abit=32):
    """3x3 convolution with padding"""
    return QConv2d(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=False, wbit=wbit, abit=abit, channel_wise=0)


class BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, wbit=32, abit=32):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, wbit=32, abit=32):
        ...
        self.conv1 = QConv2d(inplanes, planes, kernel_size=1, bias=False, wbit=wbit, abit=abit)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = QConv2d(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False, wbit=wbit, abit=abit)
        self.bn2 = nn.BatchNorm2d(planes)
        self.conv3 = QConv2d(planes, planes * 4, kernel_size=1, bias=False, wbit=wbit, abit=abit)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, wbit=32, abit=32, channel_wise=0):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.conv1 = QConv2d(3, 64, kernel_size=7, stride=2, padding=3,
                               bias=False, wbit=4, abit=4)
        ...
        self.fc = QLinear(512 * block.expansion, num_classes, wbit=wbit, abit=wbit)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, wbit=32, abit=32):
        downsample = None
